...
} as const satisfies Record<ReservationStatus, { label: string; variant: BadgeVariant }>;
...
function TableRow({ reservation }: { reservation: ReservationDto }) {
  ...
  return (
    ...
    >
      <td className="p-[var(--spacing-lg)] text-[var(--font-size-body)] font-[var(--font-weight-regular)] text-[var(--neutral-100)]">
        {formattedDate}
      </td>
      <td className="p-[var(--spacing-lg)] text-[var(--font-size-body)] font-[var(--font-weight-regular)] text-[var(--neutral-100)]">
        {formattedTime}
      </td>
      <td className="p-[var(--spacing-lg)] text-[var(--font-size-body)] font-[var(--font-weight-medium)] text-foreground">
    ...
    >
      <div className="flex justify-between items-start mb-[var(--spacing-lg)]">
        <div className="space-y-[var(--spacing-xs)]">
          <p className="text-[var(--font-size-body-large)] font-[var(--font-weight-semibold)] text-foreground group-hover:text-primary transition-colors duration-150">
            {reservation.service_name}
          </p>
          <p
            className="text-[var(--font-size-body)] font-[var(--font-weight-regular)] text-[var(--neutral-60)]"
      ...
      <div className="text-[var(--font-size-body)] text-[var(--neutral-70)] space-y-[var(--spacing-xs)]">
        <p aria-label="Data rezerwacji" className="font-[var(--font-weight-medium)]">
          {formattedDate}
        </p>
        <p aria-label="Godzina rezerwacji" className="font-[var(--font-weight-regular)]">
          {formattedTime}
        </p>
      </div>
    </div>
...
export function ReservationListItem({ reservation, view }: ReservationListItemProps) {
  return view === "table" ? <TableRow reservation={reservation} /> : <Card reservation={reservation} />;
}
