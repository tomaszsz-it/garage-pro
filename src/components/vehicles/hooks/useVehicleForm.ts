...
export function useVehicleForm({
  ...
  onSuccess,
  onError,
}: UseVehicleFormParams): UseVehicleFormResult {
  ...
  const validateField = useCallback((field: keyof VehicleFormData): string | undefined => {
  ...
  const validateForm = useCallback((): boolean => {
    ...
    // Validate all fields
    (Object.keys(formData) as Array<keyof VehicleFormData>).forEach(field => {
  ...
  const submitForm = useCallback(async (licensePlate?: string) => {
    if (!validateForm()) {
      return;
    }
    ...
    try {
      const endpoint = mode === "create" 
        ? "/api/vehicles"
        : `/api/vehicles/${encodeURIComponent(licensePlate!)}`;
